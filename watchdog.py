...
# 시작할 자동매매 프로세스 정보
WORKER_SCRIPT = os.path.join(base_path, 'worker.py')  # 별도의 워커 스크립트 필요
MARKETS = ['BTC', 'USDT', 'XRP']  # 모니터링할 코인들
...
def restart_worker(market):
    ...
    try:
        ...
        # 워커 스크립트 실행 (별도 프로세스로)
        cmd = [python_exe, WORKER_SCRIPT, '--market', market]
